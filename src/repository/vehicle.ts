...
// same Life Core primitives every other association already uses: an `about`
// Link from the item to a Vehicle Entity's own anchor. No parallel model.
//
// `links` has no constraint stopping a second `about` link to a different
// Vehicle from the same item, so "the" Vehicle is never assumed — it is
// resolved here, the same none/one/ambiguous shape `sessionControlsOf`
...
/** The `about` links from an item to a Vehicle Entity — never any other kind. */
function vehicleLinksOf(links: readonly Link[], entities: readonly Entity[], itemId: string): Link[] {
  const vehicles = vehicleItemIds(entities)
  return links.filter(
    (link) => link.kind === 'about' && link.from_id === itemId && vehicles.has(link.to_id),
  )
}

/** The one Vehicle an item points at, or why there is no such thing. */
export function vehicleLinkOf(
  links: readonly Link[],
  entities: readonly Entity[],
  itemId: string,
): VehicleLink {
  const found = vehicleLinksOf(links, entities, itemId)
  const only = found[0]
...
/** Every `about` link from an item to a Vehicle — for replacing them, not reading "the" one. */
export function vehicleLinkIdsOf(
  links: readonly Link[],
  entities: readonly Entity[],
  itemId: string,
): string[] {
  return vehicleLinksOf(links, entities, itemId).map((link) => link.id)
}
...
export function fuelRateForVehicle(
  ...
  const mine = expenses.filter((expense) => {
    ...
    const own = vehicleLinkOf(links, entities, expense.item_id)
