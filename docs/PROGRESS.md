...
- `PARTIAL` — Work → Gig Work → Multi-App Delivery (Workday recuperat cu lifecycle clar Draft/Completed pe item.state existent, Save draft cu live preview, Complete/Delete workday blocate corect de o sesiune deschisă, fuel și vehicle rate legate amândouă de Vehiculul folosit — niciodată de Arie —, ambele cu istoric propriu (`vehicle_fuel_rates`/`vehicle_cost_rates`), Vehiculul folosit este el însuși deferred la Save draft/Discard ca orice alt câmp, Complete Workday cere un Vehicul neambiguu, citiri de odometru cu finalul strict peste început și cel puțin un câștig efectiv, Completed arată numai rata pinuită și e imutabil impus acum la nivel de bază (trigger-e, extinse acum și la un Expense de cost-de-drum legat de Workday, și la ancora `items` însăși — title/due/area_id/state), nu doar în UI; Save Workday e acum atomic ca acțiune completă (item patch + restul, o singură tranzacție); un câmp legacy (Uber Eats/Deliveroo/Just Eat/Other) apare doar dacă tura are deja un câștig real pe el, un Draft nou oferă doar Platforme configurabile — dar platformele configurabile ca payout/cash-out/earning cycle rămân TODO, vezi criteriul suplimentar de mai jos; are o fundație de date aplicată live manual (vezi `docs/MIGRATII.md` pentru drift), pentru D2/D3);
...
- `TODO` — platformele Multi-App Delivery sunt records configurabile, cu earning cycle, payout schedule, cash-out behaviour și fees configurabile/effective-dated, fără cazuri Uber/Deliveroo/Just Eat hardcodate. Tabelul `platforms` (ancorat ca `entities`/`links`) și `shift_earnings.platform_item_id` există în schema/repository/cache, **aplicate live** (`20260907000000_delivery_data_foundation`, manual — vezi `docs/MIGRATII.md`). **Firul UI → repository → Supabase există acum pentru earning-uri**: `ShiftEarnings` arată un câmp pentru fiecare Platformă activă a utilizatorului (plus una dezactivată dacă tura curentă îi are deja un câștig), Save draft/Complete scriu prin `platform_item_id`, live preview include suma — nimic hardcodat Uber/Deliveroo/Just Eat în acest fir. `platform_rules` poate acum reprezenta și o payout destination reference (text simplu), alături de restul câmpurilor de regulă. Rămân neconfigurabile prin UI: earning cycle, payout schedule, cash-out, fees, payout destination, și istoricul lor effective-dated (D2). Criteriul rămâne `TODO` până acestea există.
